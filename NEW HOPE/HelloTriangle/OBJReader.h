...
private:
	...
	Index* buildIndex(vector<string> sLine) {
		return new Index(atoi(sLine[1].c_str())-1, atoi(sLine[2].c_str())-1, atoi(sLine[3].c_str())-1);
	}
		...
		while (getline(*file, line)) {
			...
			if (sLine.size() > 0 && !line.empty() && !isComentario(sLine)) {
				if (isVertice(sLine)) {
					...
					Index* aux = buildIndex(sLine);
					
					mesh->getAllIndex()->push_back(aux->getX());
					mesh->getAllIndex()->push_back(aux->getY());
					mesh->getAllIndex()->push_back(aux->getZ());
					group->getIndex()->push_back(aux->getX());
					group->getIndex()->push_back(aux->getY());
					group->getIndex()->push_back(aux->getZ());
					group->setSizeIndex(group->getSizeIndex() + 3);
				}
