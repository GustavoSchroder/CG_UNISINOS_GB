...
private:
	GLint coord[3];
public:
	Index(GLint x, GLint y, GLint z) {
		coord[0] = x;
		coord[1] = y;
		coord[2] = z;
	};
	void update(GLint x, GLint y, GLint z) {
		coord[0] = x;
		coord[1] = y;
		coord[2] = z;
	}
...
private:
	...
	bool enable;
public:
	Group(string nme, string mat, vector<Vertex*> *v, vector<Vertex*> *n, vector<TextureMapping*> *m, vector<unsigned int> *i) {
		name = nme;
		material = mat;
		vertex = v;
		normals = n;
...
private:
	...
	vector<unsigned int>* allIndex;
public:
	Mesh(vector<Group*> *g, vector<Vertex*> *v, vector<Vertex*> *n, vector<TextureMapping*> *m, vector<unsigned int> *ai, int sg) {
		groups = g;
		allVertex = v;
		allNormals = n;
		allMappings = m;
	...
	Mesh() {
		...
		allIndex = new vector<unsigned int>();
		sizeGroups = 0;
	}
	...
	vector<unsigned int>* getAllIndex() {
		return allIndex;
	}
