...
void setupMesh(unsigned int &VAO, vector<Vertex*> *allVertex,
	vector<Vertex*> *allIndex, vector<Vertex*> *allNormals, vector<TextureMapping*> *allMappings);
class Model
{
public:
	/*  Model Data */
	...
	{	
		...
		{
		setupMesh(VAO, objetos.at(i)->getAllVertex(), objetos.at(i)->getAllIndex(), objetos.at(i)->getAllNormals(), objetos.at(i)->getAllMappings());
		...
		glDrawElements(GL_TRIANGLES, objetos.at(i)->getAllIndex()->capacity(), GL_UNSIGNED_INT, 0);
		glBindVertexArray(0);
	...
	vector<Vertex*> *allIndex, vector<Vertex*> *allNormals, vector<TextureMapping*> *allMappings)
{
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, allIndex->capacity() * sizeof(Vertex), allIndex, GL_STATIC_DRAW);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vVBO);
	glBufferData(GL_ARRAY_BUFFER, allNormals->capacity() * sizeof(Vertex), allVertex, GL_STATIC_DRAW);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, tVBO);
	glBufferData(GL_ARRAY_BUFFER, allMappings->capacity() * sizeof(Vertex), allMappings, GL_STATIC_DRAW);
